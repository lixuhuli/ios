// Markup.h: interface for the CMarkupEx class.
//
// Markup Release 9.0
...
namespace PublicLib{
	...
	public:
		...
		CMarkupEx() { SetDoc(NULL); InitDocFlags(); };
		CMarkupEx(MCD_CSTR szDoc) { SetDoc(szDoc); InitDocFlags(); };
		CMarkupEx(int nFlags) { SetDoc(NULL); m_nFlags = nFlags; };
		CMarkupEx(const CMarkupEx& markup) { *this = markup; };
		void operator=(const CMarkupEx& markup);
		~CMarkupEx() {};
