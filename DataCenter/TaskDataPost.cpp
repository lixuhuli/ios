...
namespace DataCenter{
	...
	int CTaskDataPost::Run()
	{
		...
		http.AddHttpHeader(L"X-WuFan-Client-Token", CDataCenter::Instance()->GetToken().c_str());
		Json::Value vData;
		vData[KEY_DEVICE_ID] = PublicLib::UToA(CDataCenter::Instance()->GetGuid());
		vData[KEY_AD_ID] = CDataCenter::Instance()->GetAdID();
		...
		vArray.append(vPost);
		Json::FastWriter fw;
		string strPost = fw.write(vArray);
