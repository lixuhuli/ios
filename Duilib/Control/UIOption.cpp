...
namespace DuiLib
{
	...
	{
		if(_tcscmp(pstrName, DUI_CTR_OPTION) == 0) 
	...
	void COptionUI::Selected(bool bSelected, bool bNotify /* = true */)
	{
	...
	void COptionUI::PaintStatusImage(HDC hDC)
	{
		...
		if( (m_uButtonState & UISTATE_HOT) != 0 && IsSelected() && !m_sSelectedHotImage.IsEmpty()) {
			...
			if (!m_sSelectedBkImage.IsEmpty())
				DrawImage(hDC, (LPCTSTR)m_sSelectedBkImage);
	...
	void COptionUI::PaintText(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 )
		{
			...
			if( m_bShowHtml )
				...
				CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, IsEnabled()?m_dwTextColor:m_dwDisabledTextColor, m_iFont, m_uTextStyle, m_bDrawTextPlus);
