...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
    ...
    m_ListInfo.dwCheckedTextColor = 0;
...
void CListUI::SetCheckedItemTextColor(DWORD dwCheckTextColor)
{
    m_ListInfo.dwCheckedTextColor = dwCheckTextColor;
    Invalidate();
}
...
void CListUI::LineUp(int nLine/* = 1*/)
{
    m_pList->LineUp(nLine);
}

void CListUI::LineDown(int nLine/* = 1*/)
{
    m_pList->LineDown(nLine);
}
...
void CListElementUI::SetBtnState( UINT nState )
{
	m_uButtonState = nState;
	Invalidate();
}

UINT CListElementUI::GetBtnState()
{
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_KEYDOWN && IsEnabled() )
    {
		m_pManager->SendNotify(this, DUI_MSGTYPE_KEYDOWN, event.chKey, event.wKeyState);
        if( event.chKey == VK_RETURN ) {
            Activate();
            Invalidate();
            return;
...
{
	//OutputDebugString(_T("DrawItemBk"));
...
void CListContainerElementUI::SetBtnState( UINT nState )
{
	m_uButtonState = nState;
	Invalidate();
}

UINT CListContainerElementUI::GetBtnState()
{
	return m_uButtonState;
}
