...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
        ...
        if (event.Type == UIEVENT_TIMER)
        {
            ...
            //if (event.wParam == BEGIN_HOT_TIMER)
            //{
            //    m_nCount += GROWTH_NUM;
            //    if (m_nCount > 255) m_nCount = 255;
            //    CRenderEngine::DrawImageString2(GetManager()->GetPaintDC(), m_pManager, m_rcItem, m_rcPaint, m_sHotImage, NULL, this, m_nCount);
            //}

            //if (event.wParam == END_HOT_TIMER)
            //{
            //    if (m_nCount > 100)
            //    {
            //        m_nCount -= UNGROWTH_NUM;
            //        CRenderEngine::DrawImageString2(GetManager()->GetPaintDC(), m_pManager, m_rcItem, m_rcPaint, m_sHotImage, NULL, this, m_nCount);
            //    }
            //    else
            //    {
            //        GetManager()->KillTimer(this, END_HOT_TIMER);
            //        m_IsButtonHot[1] = FALSE;
            //        DrawImage(GetManager()->GetPaintDC(), (LPCTSTR)m_sNormalImage);
            //    }
            //}
        }
		...
		if( event.Type == UIEVENT_SETCURSOR && m_bShowCursor) {
	...
	void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if ( _tcscmp(pstrName, _T("showcursor")) == 0 )
		{
	...
	void CButtonUI::PaintText(HDC hDC)
	{
		...
		if( m_bShowHtml )
			...
			CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, clrColor, m_iFont, m_uTextStyle, m_bDrawTextPlus);
	}
...
Label_ForeImage:
	...
	void CButtonUI::SetUser64Data(__int64 nData)
	{
		m_nUserData = nData;
	}

	__int64 CButtonUI::GetUser64Data()
	{
		return m_nUserData;
	}

	void CButtonUI::SetShowCursor(bool bShow)
	{
		m_bShowCursor = bShow;
	}

    bool CButtonUI::OnFocusOutAnimationEnd()
    {
        m_uButtonState &= ~UISTATE_HOT;
    ...
    {
        if (!m_sNormalImage.IsEmpty() && GetFadeEffectProperty() == 4 && (m_uButtonState & UISTATE_HOT) == UISTATE_HOT)
