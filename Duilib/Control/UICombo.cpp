...
public:
	...
	virtual bool OnNotifyRoot(void* pParam);
...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    rc.bottom = rc.top + MIN(cyFixed, szDrop.cy);
	 ...
	 if(pOwner->IsItemPopTop())
	 {
		 rc.bottom = rcOwner.top;
		 rc.top = rc.bottom - (rcTmp.bottom - rcTmp.top);
	 }
...
bool CComboWnd::OnNotifyRoot(void* pParam)
{
	TNotifyUI* pNotify = (TNotifyUI*)pParam;
	if(pNotify->sType != DUI_MSGTYPE_WINDOWINIT) return true;
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        m_pLayout->SetBkColor(m_pOwner->GetDropBoxBkColor());
        m_pLayout->SetBorderColor(m_pOwner->GetDropBoxBoardColor());
        m_pLayout->SetBorderSize(1);
        m_pLayout->SetAutoDestroy(false);
        m_pLayout->EnableScrollBar();
        m_pLayout->ApplyAttributeList(m_pOwner->GetDropBoxAttributeList());
		m_pLayout->OnNotify += MakeDelegate(this, &CComboWnd::OnNotifyRoot);
		...
		m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_ONDESTROYDROPDOWN);
...
, m_dwDropBoxBoardColor(0xFFC6C7D2)
{
...
bool CComboUI::SelectItemV2( int iIndex, bool bTakeFocus /*= false*/, bool bNotify /*= false*/ )
{
	if( m_pWindow != NULL ) m_pWindow->Close();
	if( iIndex == m_iCurSel ) return true;
	int iOldSel = m_iCurSel;
	...
	if( m_pWindow != NULL && bTakeFocus ) pControl->SetFocus();
	pListItem->Select(true);
	if( m_pManager != NULL && bNotify) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, m_iCurSel, iOldSel);
	Invalidate();
...
bool CComboUI::Activate()
{
	...
	if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_BEFOREDROPDOWN);
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("dropboxbkcolor")) == 0)
	{
		if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		m_dwDropBoxBkColor = _tcstoul(pstrValue, &pstr, 16);
	}
	else if (_tcscmp(pstrName, _T("dropboxboardcolor")) == 0)
	{
		if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		m_dwDropBoxBoardColor = _tcstoul(pstrValue, &pstr, 16);
	}
	...
	else if( _tcscmp(pstrName, _T("itemtop")) == 0 ) m_bItemPopTop = (_tcscmp(pstrValue, _T("true")) == 0);
...
bool CComboUI::IsItemPopTop()
{
	return m_bItemPopTop;
}

void CComboUI::ButtonStateChanged()
{
	if(m_pManager) m_pManager->SendNotify(this, DUI_MSGTYPE_BUTTONSTATECHANGED);
}

UINT CComboUI::GetButtonState()
{
