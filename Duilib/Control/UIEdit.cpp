...
namespace DuiLib
{
	...
	public:
		...
		HWND Init(HWND hParentWnd, CEditUI* pOwner);
		RECT CalPos();
	...
	HWND CEditWnd::Init(HWND hParentWnd, CEditUI* pOwner)
	{
		...
		Create(m_hParentWnd, NULL, uStyle, 0, rcPos);
		HFONT hFont=NULL;
	...
	RECT CEditWnd::CalPos()
	{
        ...
        SIZE szOffset = m_pOwner->GetWndOffset();
		rcPos.left += (rcInset.left + szOffset.cx);
		rcPos.top += (rcInset.top + szOffset.cy);
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if( _tcscmp(pstrName, _T("wndoffset")) == 0 ) {
            LPTSTR pstr = NULL;
...
Label_ForeImage:
    ...
    void CEditUI::SetWndOffset(int cx, int cy)
    {
        m_szWndOffset.cx = cx;
        m_szWndOffset.cy = cy;
    }

    SIZE CEditUI::GetWndOffset() const
    {
        return m_szWndOffset;
    }
