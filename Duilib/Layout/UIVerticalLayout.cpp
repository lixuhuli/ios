...
namespace DuiLib
{
	CVerticalLayoutUI::CVerticalLayoutUI() : m_iSepHeight(0), m_uButtonState(0), m_bImmMode(false), vscroll_dis_side_(-1)
	{
	...
	void CVerticalLayoutUI::SetPos(RECT rc)
	{
        ...
        auto rc_v_bar = rc;
        if (vscroll_dis_side_ >= 0) {
            rc_v_bar.left -= m_rcInset.left;
            rc_v_bar.top -= m_rcInset.top;
            rc_v_bar.right += (m_rcInset.right - vscroll_dis_side_);
            rc_v_bar.bottom += m_rcInset.bottom;
        }
	...
	void CVerticalLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcscmp(pstrName, _T("vscrolltoside")) == 0) SetVscrollToside(_ttoi(pstrValue));
    ...
    void CVerticalLayoutUI::SetVscrollToside(int distance) {
        vscroll_dis_side_ = distance;
    }

    int CVerticalLayoutUI::GetVscrollToside() const {
        return vscroll_dis_side_;
    }
