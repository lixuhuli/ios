...
namespace DuiLib
{
	...
	bool CTabLayoutUI::SelectItem(int iIndex, bool bSetFocus /*= true*/)
	{
		...
		for( int it = 0; it < m_items.GetSize(); it++ )
		{
			if( it == iIndex ) 
			{
				...
				if(bSetFocus) GetItemAt(it)->SetFocus();
	...
	{
		if( _tcscmp(pstrName, _T("selectedid")) == 0 ) SelectItem(_ttoi(pstrValue), false);
