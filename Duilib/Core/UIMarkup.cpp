...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    ...
    if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
        ...
        DuiHZIP hz = NULL;
        if( CPaintManagerUI::IsCachedResourceZip() ) hz = (DuiHZIP)CPaintManagerUI::GetResourceZipHandle();
        else hz = DuiOpenZip((void*)sFile.GetData(), 0, 2);
        if( hz == NULL ) return _Failed(_T("Error opening zip file"));
        DuiZIPENTRY ze; 
        int i; 
		CDuiString strFileName = pstrFilename;
		if(!CPaintManagerUI::GetResourceZipBaseDepth().IsEmpty()) strFileName = CPaintManagerUI::GetResourceZipBaseDepth() + pstrFilename;
        if( DuiFindZipItem(hz, strFileName.GetData(), true, &i, &ze) != 0 ) return _Failed(_T("Could not find ziped file"));
        DWORD dwSize = ze.unc_size;
        if( dwSize == 0 ) return _Failed(_T("File is empty"));
        if ( dwSize > 4096*1024 ) return _Failed(_T("File too large"));
        BYTE* pByte = new BYTE[ dwSize ];
        int res = DuiUnzipItem(hz, i, pByte, dwSize, 3);
        if( res != 0x00000000 && res != 0x00000600) {
            delete[] pByte;
            if( !CPaintManagerUI::IsCachedResourceZip() ) DuiCloseZip(hz);
