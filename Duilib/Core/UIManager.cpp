...
CDuiString CPaintManagerUI::m_pStrResourceZipBaseDepth;
...
m_bAlphaDisplay(false),
m_bUsedVirtualWnd(false),
m_nOpacity(255),
m_pParentResourcePM(NULL),
m_pBmpBackgroundBits(NULL),
m_bLayeredWithPicture(false),
m_fUpdateLayeredWindow(false),
m_bLayered(false),
m_aAlpha(255)
{
...
void CPaintManagerUI::SetResourceZip(LPVOID pVoid, unsigned int len)
{
    ...
    if( m_bCachedResourceZip && m_hResourceZip != NULL ) {
        DuiCloseZip((DuiHZIP)m_hResourceZip);
        m_hResourceZip = NULL;
    }
    m_pStrResourceZip = _T("membuffer");
    m_bCachedResourceZip = true;
    if( m_bCachedResourceZip ) 
        m_hResourceZip = (HANDLE)DuiOpenZip(pVoid, len, 3);
}
...
void CPaintManagerUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip)
{
    ...
    if( m_bCachedResourceZip ) {
        ...
        m_hResourceZip = (HANDLE)DuiOpenZip((void*)sFile.GetData(), 0, 2);
    }
}
...
void CPaintManagerUI::SetDisplayTransparent(bool bTrans) {
    m_bAlphaDisplay = bTrans;
}
    ...
    case WM_PAINT:
        {
			...
			{
				PaintAlphaBackground(rcPaint, rcClient);
				return true;
			}           
            ...
            if( m_bOffscreenPaint && m_hbmpOffscreen == NULL )
            {
                ...
                if (m_bLayered) {
                    m_hbmpOffscreen = CGdiAlpha::CreateBitmap32(m_hDcPaint, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top);
                }
                else {
                ...
                if (m_hDcOffscreen == nullptr || m_hbmpOffscreen == nullptr) {
                    PAINTSTRUCT ps = { 0 };
                    ::BeginPaint(m_hWndPaint, &ps);
                    ::EndPaint(m_hWndPaint, &ps);
                    return true;
            ...
            PAINTSTRUCT ps = { 0 };
            ::BeginPaint(m_hWndPaint, &ps);
            if( m_bOffscreenPaint && m_hDcOffscreen)
            {
                HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(m_hDcOffscreen, m_hbmpOffscreen);
                int iSaveDC = ::SaveDC(m_hDcOffscreen);

                if (m_bAlphaBackground && !m_bLayered) {
                    if (m_hbmpBackground == NULL) {
                        RECT rcClient = { 0 };
                        ::GetClientRect(m_hWndPaint, &rcClient);
                ...
                if (m_bLayered) {
                    //如窗口隐藏，窗口需要再次显示前调用ClearLayeredStyle函数，否则不能产生WM_PAINT消息
                    UpdateLayeredUI(ps.rcPaint);
                }
                else {
                    ::BitBlt(ps.hdc, ps.rcPaint.left, ps.rcPaint.top,
                        ps.rcPaint.right - ps.rcPaint.left,
                        ps.rcPaint.bottom - ps.rcPaint.top, m_hDcOffscreen, ps.rcPaint.left,
...
void CPaintManagerUI::SetFocus(CControlUI* pControl, bool bFocusWnd /*= true*/)
{
    // Paint manager window has focus?
    HWND hFocusWnd = ::GetFocus();
    if( hFocusWnd != m_hWndPaint && pControl != m_pFocus && bFocusWnd) ::SetFocus(m_hWndPaint);
    // Already has focus?
...
//     if( pControl != NULL ) 
// 	{
// 		LPCTSTR pstrCls = pControl->GetClass();
// 		if(0 != _tcscmp(pstrCls, _T("EditUI")))//不对edit控件设置焦点
// 		{
// 			bool bKeepLastFocus = false;
// 			if(m_pFocus)
// 			{
// 				LPCTSTR pstrFocusClass = m_pFocus->GetClass();
// 				if(0 == _tcscmp(pstrCls, _T("EditUI")))
// 				{
// 					CEditUI* pEdit = (CEditUI*)m_pFocus;
// 					if(pEdit->IsKeepFocusWhenTabSwitch())//如果有该属性,就keepfocus
// 						bKeepLastFocus = true;
// 				}
// 			}
// 			if(bKeepLastFocus) 
...
HFONT CPaintManagerUI::AddFont(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bStrikeout)
{
    LOGFONT lf = { 0 };
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
    _tcsncpy(lf.lfFaceName, pStrFontName, LF_FACESIZE);
...
const TImageInfo* CPaintManagerUI::GetImageFromBuffer(LPCTSTR bitmap, LPCSTR pstrBuffer, DWORD dwSize, DWORD mask)
{
	TImageInfo* data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
	if( !data )
	{
		if( AddImage(bitmap, (LPBYTE)pstrBuffer, dwSize, mask) ) 
		{
	...
	if (NULL == data)
	{
		DuiLib::DUI__Trace(_T("图片标志为：%s"), bitmap);
		DuiLib::DUI__Trace(_T("啦啦啦：%s"), bitmap);
	}
...
{
	if( pData == NULL || dwSize <= 0 || bitmap == 0 ) return NULL;
...
const void CPaintManagerUI::SetResourceZipBaseDepth(LPCTSTR pStrFolder)
{
	m_pStrResourceZipBaseDepth = pStrFolder;
	if(!m_pStrResourceZipBaseDepth.IsEmpty() && m_pStrResourceZipBaseDepth.GetAt(m_pStrResourceZipBaseDepth.GetLength() - 1) != L'\\') m_pStrResourceZipBaseDepth += L"\\";
}

const CDuiString& CPaintManagerUI::GetResourceZipBaseDepth()
{
	return m_pStrResourceZipBaseDepth;
}

bool CPaintManagerUI::SetXmlTextPath(LPCTSTR pStrPathInResource)
{
	return CXmlParser::Instance()->LoadStaticXmlFile(pStrPathInResource);
}

void CPaintManagerUI::UpdateEventHover( CControlUI* pNewHover )
{
	m_pEventHover = pNewHover;
}

void CPaintManagerUI::SetLayeredWithPicture(bool bLayered) {
    m_bLayeredWithPicture = bLayered;
    SetLayered(bLayered);
}

bool CPaintManagerUI::GetLayeredWithPicture() {
    return m_bLayeredWithPicture;
}
...
bool CPaintManagerUI::GetLayered() const {
    return m_bLayered;
}

void CPaintManagerUI::SetAlphaChannel(BYTE alpha) {
    m_aAlpha = alpha;
    SetLayered(true);
}

BYTE CPaintManagerUI::GetAlphaChannel() const {
    return m_aAlpha;
}

BOOL CPaintManagerUI::UpdateLayeredUI(RECT rcPaint) {
    if (NULL == m_fUpdateLayeredWindow) {
        HMODULE hUser32 = ::GetModuleHandle(_T("User32.dll"));
        if (hUser32) {
            m_fUpdateLayeredWindow = (PFUNCUPDATELAYEREDWINDOW)::GetProcAddress(hUser32,
                "UpdateLayeredWindow");
            if (NULL == m_fUpdateLayeredWindow) {
                return false;
            }
    ...
    RECT rtwnd;
    GetWindowRect(m_hWndPaint, &rtwnd);
    POINT ptSrc;
    ptSrc.x = rcPaint.left;
    ptSrc.y = rcPaint.top;
    SIZE sz;
    ...
    ptDest.x = rtwnd.left;
    ptDest.y = rtwnd.top;
    HDC hScreenDC = GetWindowDC(m_hWndPaint);
    DWORD dwExStyle = ::GetWindowLong(m_hWndPaint, GWL_EXSTYLE);
    if ((dwExStyle & 0x80000) != 0x80000) {
        ::SetWindowLong(m_hWndPaint, GWL_EXSTYLE, dwExStyle ^ 0x80000);
    }
    BLENDFUNCTION bf = { AC_SRC_OVER, 0, m_aAlpha, AC_SRC_ALPHA };
    HDC hDesktopDc = GetDC(NULL);
    BOOL bRes = m_fUpdateLayeredWindow(m_hWndPaint, hDesktopDc, &ptDest, &sz,
        m_hDcOffscreen, &ptSrc, 0, &bf, ULW_ALPHA);
    if (m_hbmpOffscreen != NULL) {
        ::DeleteObject(m_hbmpOffscreen);
    }
    if (m_hDcOffscreen != NULL) {
    ...
    ReleaseDC(m_hWndPaint, hScreenDC);
    return bRes;
}

bool CPaintManagerUI::ClearBmpAlpha(HBITMAP bmp) {
    BITMAP  bm;
    GetObject(bmp, sizeof(BITMAP), &bm);
    if (bm.bmBitsPixel != 32 || NULL == bm.bmBits) {
        return false;
    }
    LPBYTE pBit = (LPBYTE)bm.bmBits + 3;
...
void CPaintManagerUI::PaintAlphaBackground(const RECT& rcPaint, const RECT& rcClient)
{
	DWORD dwExStyle = GetWindowLong(m_hWndPaint, GWL_EXSTYLE);
	...
	{
		int x = (i * width + j) * 4;
        if (m_pBmpBackgroundBits[x + 0] != 0
            || m_pBmpBackgroundBits[x + 1] != 0
            || m_pBmpBackgroundBits[x + 2] != 0) {
            if (m_bAlphaDisplay || m_pBmpBackgroundBits[x + 3] == 0) 
