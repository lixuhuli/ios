...
CDialogBuilder::CDialogBuilder() : m_pCallback(NULL), m_pstrtype(NULL), m_pInheritableTag(NULL)
{

}

CControlUI* CDialogBuilder::Create(STRINGorID xml, LPCTSTR type, IDialogBuilderCallback* pCallback, 
                                   CPaintManagerUI* pManager, CControlUI* pParent, LPCTSTR szInheritableUserData /*= NULL*/, UINT_PTR pInheritableTag /*= NULL*/)
{
	...
	if(szInheritableUserData) m_sInheritableUserData = szInheritableUserData;
	if(pInheritableTag) m_pInheritableTag = pInheritableTag;
        ...
        if( _tcscmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    else if (_tcscmp(pstrName, _T("bkdisplay")) == 0) {
                        pManager->SetDisplayTransparent(_tcscmp(pstrValue, _T("true")) == 0);
                    }
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        if( _tcscmp(pstrClass, _T("Image")) == 0 || _tcscmp(pstrClass, _T("Font")) == 0 \
			...
			||  _tcscmp(pstrClass, _T("XmlText")) == 0) continue;
		...
		//树控件XML解析
			...
			if( pManager ) 
			{
				pNode->SetManager(NULL, NULL, false);
			}
            ...
            case 15:
				...
				else if (_tcscmp(pstrClass, DUI_CTR_ANIMATTABLAYOUT) == 0)	  pControl = new CAnimatTabLayoutUI;
                break;
            case 16:
        ...
        ASSERT(pControl);
			...
			if(!m_sInheritableUserData.IsEmpty()) pControl->SetInheritableUserData(m_sInheritableUserData);
			if(m_pInheritableTag) pControl->SetInheritableTag(m_pInheritableTag);
