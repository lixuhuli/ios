...
public:
    ...
    void SetDisplayTransparent(bool bTrans);
	...
	static const void SetResourceZipBaseDepth(LPCTSTR pStrFolder);
	static const CDuiString& GetResourceZipBaseDepth();
	...
	static bool SetXmlTextPath(LPCTSTR pStrPathInResource);
    ...
    HFONT AddFont(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bStrikeout=false);
	...
	const TImageInfo* GetImageFromBuffer(LPCTSTR bitmap, LPCSTR pstrBuffer, DWORD dwSize, DWORD mask = 0);
	...
	void UpdateEventHover(CControlUI* pNewHover);

    void SetLayeredWithPicture(bool bLayered);
    bool GetLayeredWithPicture();
    void SetLayered(bool bLayered);
    bool GetLayered() const;
    void SetAlphaChannel(BYTE alpha);
    BYTE GetAlphaChannel() const;
    bool ClearBmpAlpha(HBITMAP bmp);
    BOOL UpdateLayeredUI(RECT rcPaint);

private:
	void PaintAlphaBackground(const RECT& rcPaint, const RECT& rcClient);
...
private:
    ...
    BYTE m_aAlpha;
    bool m_bLayered;
    bool m_bLayeredWithPicture;
    typedef BOOL(__stdcall* PFUNCUPDATELAYEREDWINDOW)(HWND, HDC, POINT*, SIZE*,
        HDC, POINT*, COLORREF crKey, BLENDFUNCTION*, DWORD);
    PFUNCUPDATELAYEREDWINDOW m_fUpdateLayeredWindow;
	...
	static CDuiString m_pStrResourceZipBaseDepth;
