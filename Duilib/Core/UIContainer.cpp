...
namespace DuiLib
{
	...
	CContainerUI::CContainerUI()
        ...
        m_bScrollFloat(false),
		m_nVerticalLineStep(120),
		m_nHorizontalLineStep(8)
	{
		::ZeroMemory(&m_rcInset, sizeof(m_rcInset));
        m_iVisibleCount = 0;
        m_iFirstVisible = 0;
        m_btAlphaContainer = 255;
	}
	...
	int CContainerUI::GetItemIndex(void* pControl) const
	{
	...
	bool CContainerUI::SetItemIndex(void* pControl, int iIndex)
	{
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			if(m_items[it] == pControl) 
			{
	...
	void CContainerUI::DoEvent(TEventUI& event)
	{
		...
		if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() && m_pVerticalScrollBar->IsEnabled() )
		{
			...
			case VK_END:
                ...
                int nChars = 3;
                SystemParametersInfo(/*SPI_GETWHEELSCROLLCHARS*/0x6C, 0, &nChars, 0);
                if (nChars < 1)
                    nChars = 1;
    ...
    void CContainerUI::LineUp(int nLine/* = 1*/)
	{
        int cyLine = 8;
        ...
        {
            sz.cy -= m_pVerticalScrollBar->GetLineSize();
        }
        SetScrollPos(sz);
	}

    void CContainerUI::LineDown(int nLine/* = 1*/)
	{
        int cyLine = 8;
        if (m_pManager) cyLine = m_pManager->GetDefaultFontInfo()->tm.tmHeight + 8;
    ...
    int CContainerUI::GetFirstVisible()
    {
        return m_iFirstVisible;
    }

    int CContainerUI::GetVisibleCount()
    {
        return m_iVisibleCount;
    }

    void CContainerUI::SetTransparent(BYTE btAlpha)
    {
        if (m_aFadeEffectProperty > 0)
            m_btAlpha = btAlpha;
        m_btAlphaContainer = btAlpha;
    }
	...
	void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcscmp(pstrName, _T("scrollfloat")) == 0) SetScrollFloat(_tcscmp(pstrValue, _T("true")) == 0);
	...
	void CContainerUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
        ...
        int iFirstVisible = 0, iVisibleCount = 0;
        if (m_btAlphaContainer < 255)
        {
            CDrawingBoard board;
            QRect rcClient;
            ::GetClientRect(m_pManager->GetPaintWindow(), &rcClient);
            board.Create(RECT_WIDTH(rcClient), RECT_HEIGHT(rcClient));
            ActualDoPaint(board.GetSafeHdc(), rcTemp, rcPaint, iFirstVisible, iVisibleCount);
            board.AlphaBlendTo(hDC, m_rcItem, m_rcItem, (int)m_btAlphaContainer, true);
        }
        else
            ActualDoPaint(hDC, rcTemp, rcPaint, iFirstVisible, iVisibleCount);

        if (m_iFirstVisible != iFirstVisible &&
            iVisibleCount != -1 &&
            iFirstVisible != -1 &&
            iFirstVisible < m_items.GetSize())
        {
            m_iFirstVisible = iFirstVisible;
            m_iVisibleCount = iVisibleCount;
            m_pManager->SendNotify(this, L"itemvisiblechange", m_iFirstVisible, m_iVisibleCount);
            OnFirstVisibleChange(hDC, m_iFirstVisible, m_iVisibleCount);
        }
	}

    void CContainerUI::ActualDoPaint(HDC hDC, RECT& rcTemp, const RECT& rcPaint, int& iFirstVisible, int& iVisibleCount)
    {
        CRenderClip clip;
        ...
        if (m_items.GetSize() > 0)
        {
            ...
            rc.right -= m_rcInset.right;
            rc.bottom -= m_rcInset.bottom;
            if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() && !m_bScrollFloat) rc.right -= m_pVerticalScrollBar->GetFixedWidth();
            if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() && !m_bScrollFloat) rc.bottom -= m_pHorizontalScrollBar->GetFixedHeight();
            ...
            if (!::IntersectRect(&rcTemp, &rcPaint, &rc)) {
                for (int it = 0; it < m_items.GetSize(); it++) {
                    ...
                    rcCtrlPos = pControl->GetPos();
                    if (!pControl->IsFloat() && PtInRect(&m_rcItem, QPoint(rcCtrlPos.left, rcCtrlPos.bottom)))
                    {
                        if (bFirstVisible)
                        {
                            bFirstVisible = FALSE;
	...
	{
        do {
            if (m_pHorizontalScrollBar == NULL) break;
	...
	void CContainerUI::SetVerticalLineStep( int nStep )
	{
		m_nVerticalLineStep = nStep;
	}

	void CContainerUI::SetHorizontalLineStep( int nStep )
	{
		m_nHorizontalLineStep = nStep;
	}

	int CContainerUI::GetVerticalLineStep()
	{
		return m_nVerticalLineStep;
	}

	int CContainerUI::GetHorizontalLineStep()
	{
		return m_nHorizontalLineStep;
	}
